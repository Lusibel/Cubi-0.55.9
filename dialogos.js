...
function mostrarDialogo(npc){
    ...
    document.getElementById("dialogNameBoss").textContent =
        dialogosActuales[dialogoActual].nombreboss;
        document.getElementById("dialogNameFantasma").textContent =
        dialogosActuales[dialogoActual].nombreFantasma;
...
function siguienteDialogo() {
    ...
    if (dialogoActual >= dialogosActuales.length) {
        ...
        // Si era un jefe, iniciar combate
        if (
            npcActual &&
            npcActual.esJefe &&
            currentBossRoom
